...
public class GetRandomWordCommand : ICommand
{
    public GetRandomWordCommand(IUserRepo userRepo, IWordsApiRepo wordsApiRepo, IWordRepo wordRepo)
    {
        _userRepo = userRepo;
    ...
    public string Key => Commands.GetRandomWordCommand;
    ...
    public async Task Execute(Update? update, ITelegramBotClient client)
    {
        ...
        if (userData != null)
        {
            ...
            {
                case 0:
                    if (text == Key || text == CommandTypes.TryAgain)
                    {
                        var wordInfo = _wordsApiRepo.GetRandomWordInfo().Result;
                        sentMessage = await client.SendTextMessageAsync(message.Chat.Id, "loading...");
                        if (wordInfo == null || wordInfo.Results == null)
                        {
                            while (wordInfo == null || wordInfo.Results == null)
                            {
                                wordInfo = _wordsApiRepo.GetRandomWordInfo().Result;
                            }
                        }
